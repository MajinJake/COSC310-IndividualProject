...
| Feature    | Description |
| ---------- | ----------------------------------------------------------------------- |
| Wikipedia API | The chatbot can now look up wikipedia pages. Typing "What is x" will bring up the topic's wikipedia link and wikipedia summary! ![Imgur](https://i.imgur.com/gWapgdb.png) |
| Bing News API | The chatbot can now look up the top news headlines on bing! Use it by typing "I have nothing to do" into the chatbot ![Imgur](https://i.imgur.com/rqAvUYk.png) |
...
To install wikipedia enter pip install -wikipedia into terminal/powershell
...
- **WikipediaAPI:** 
A class that has a single method. The method, wikipage, takes in one input, a string. This string uses the wikipedia API to return a string of the title, url, and the summary of the inputted string. 
- **BingNewsAPI:** 
A class that has a single method. The method, headlines, takes in no parameters and uses the Bing News API to return an array of the top 5 news articles on Bing. Each index of the array has the headline and the URL of the news article.
- **BotRespons:** 
